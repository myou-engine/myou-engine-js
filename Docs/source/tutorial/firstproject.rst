...
Skip to the `coding`_


Firstly, make a new folder called "myouprojects". This is where all your projects will go.

Then, go into your project folder and run the following command in your terminal, command prompt, or Nodejs Command Prompt (I will refer to all of these as terminal from now on): :code:`npm install --save myou-engine`

You should also type :code:`npm install -g webpack && npm install -g shelljs` if you don't have that yet. It's how we will build everything into a working project that can be seen in a browser.
...
Immediately, go to File > Export > Myou Engine. Export it into your rotatingcube directory under the name :code:`data`. You should now have a new folder named :code:`data` inside your rotatingcube directory.

Now, make a file called :code:`main.coffee`. This is where the bulk of your code will go and will probably be the only thing you need to keep open.
...
Make a new folder in your rotatingcube folder, name it :code:`static_files`. Inside the new folder, make a file called :code:`myou.html`. Not much has to go in here, but it's still imperative to making the project viewable.
...
The :code:`<script>` tag is what HTML uses to recognize when Javascript is being written, or loaded from an external file. In this case, it's being loaded from the file: :code:`myou.js`.
...
Now, open :code:`main.coffee`. Bask in its blankness. The infinite nothing of the empty file. As beautiful as the emptiness may be, it's time to fill it up with something.
...
:code:`myou_params` is a set of arguments that you can set for the Myou engine. You can see the list of arguments that can be set `here <../myou/myouparams.html>`_
...
the indent before :code:`toys` tells us that they are inside of the function :code:`box()`.

After the :code:`=`, you'll notice an arrow. That arrow tells us what the function :code:`box()` will return.
...
What this does is use `quaternions <https://en.wikipedia.org/wiki/Quaternion>`_ to rotate the cube, and it looks like this:

.. image:: ./images/SpinningStartingCube.gif

First, it saves the orientation of the cube to the variable :code:`q`. This serves no functional purpose, but makes writing it much, much easier and quicker. Otherwise, the two lines would only be one that read
::

  quat.rotateY(scene.objects.Cube.rotation, scene.objects.Cube.rotation, 0.01/16*frame_duration).

Now, in the second line, ":code:`quat.rotateY(q, q, 0.01/16*frame_duration)`" works by taking, in order, the variable it will output to (q), the variable it will change (also q), and the angle of change in radians.

Further explanation on quaternions can be found `here <http://www.opengl-tutorial.org/intermediate-tutorials/tutorial-17-quaternions/>`_.

Implementation of the function :code:`quat.rotateY` can be found `here <http://glmatrix.net/docs/quat.js.html#line292>`_.

Documentation on the glMatrix :code:`quat` element can be found `here <http://glmatrix.net/docs/quat.html>`_
