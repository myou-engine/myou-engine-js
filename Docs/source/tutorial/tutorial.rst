...
- `Node.js <https://nodejs.org/en/>`_ (I recommend you use the LTS version)

- `Blender <https://www.blender.org/>`_
	- At the moment there is a slight bug with rendering hemi lighting in Blender 2.78, you may want to use an older version of Blender :title:Blender versions 2.77 and 2.77a`here <http://download.blender.org/release/Blender2.77/>`_ if you happen to use that.

Once Blender is installed, download the Myou plugin `here <https://github.com/myou-engine/myou-blender-plugin/archive/master.zip>`_ . It will have to be installed in Blender to allow you to export models for Myou. Open Blender, and go to File > User Preferences > Addons > Install from file. Here, use the zip file you downloaded.
...
3. Click "new" and add the path to the Node.js folder where the .exe is. It is typically saved to :code:`c:\Program Files\nodejs\node_modules\npm\bin`

Once you have Node set up, you can move on to the next step: `Your first project <firstproject.html>`_
